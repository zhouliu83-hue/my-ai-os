...
description: |
  ...
  触发方式：/zz-benchmark、/对标、「帮我找对标」「我该模仿谁」
  Benchmark analysis using dontbesilent's five-filter method.
  Trigger: /zz-benchmark, "find me a benchmark", "who should I copy"
---
...
| 触发条件 | 推荐话术 |
|---|---|
| 对标已经找到，下一步要进入具体表达和内容执行 | 「对标有了，下一步看内容怎么做。用 `/zz-content`。」 |
| 对标已经找到，接下来要长期跟踪自己的选择、调整和结果 | 「对标有了。后面哪些判断真的有效，记到 `/zz-decision` 里慢慢回填。」 |
| 用户找对标前还没说清楚自己要解释或改进哪个问题 | 「先把问题写成可推理的问题说明书，再决定找谁对标。用 `/zz-good-question`。」 |
| 用户总想跳过模仿颗粒度、直接抄结果或找更快的捷径 | 「你不是缺对标，是在关键判断环节想贪快。试试 `/zz-slowisfast`。」 |
| 用户反复说"不适合我"，疑似心理卡点 | 「你可能不是在选对标，是在逃避执行。试试 `/zz-action`。」 |
...
> 这次找到的对标和模仿颗粒度，输入 `/zz-save` 存档。下次想加新对标或者复盘有没有真的模仿到位，`/zz-restore` 接着上次。
