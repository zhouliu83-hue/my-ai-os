...
name: zz-report
description: |
  把多次 zz-save 攒下来的诊断状态合并成一份可交付的 markdown 报告。
  触发方式：/zz-report、/出报告、「打包」「整理一份」「给合伙人看的」
  Generate a deliverable diagnosis report by merging all zz-save snapshots.
  Trigger: /zz-report, "package this up", "make me a report"
---
...
**报告不是你从对话里凭空总结。** 你只读 `~/.zz/sessions/{项目名}/` 下的存档文件，按时间顺序合并、去重、分类。这是报告的可信度来源——它是用户已经确认过的状态的合集，不是 AI 二次发挥。
...
按项目找 `~/.zz/sessions/{项目名}/*.md`。

- 0 个文件 → 「`{项目名}` 下没有存档，先用 `/zz-save` 存几次诊断结果再来出报告。」
- 1 个文件 → 提示：「`{项目名}` 下只有 1 份存档，单份不需要合并报告。直接看 `~/.zz/sessions/{项目名}/{文件名}` 就行。」并询问「还是要强制出报告吗？」如果用户说要，继续。
- ≥2 个文件 → 直接进入合并
...
```
~/.zz/reports/{项目名}/{YYYYMMDD-HHMMSS}-{项目名}.md
```
...
|---|---|---|---|---|
| 2026-04-15 | 卖什么没想清楚 | 进行中 | zz-diagnosis | `~/.zz/sessions/{项目名}/20260415-...md` |
| ... | ... | ... | ... | ... |
...
报告由 zzskill 自动生成。原始存档见 `~/.zz/sessions/{项目名}/`。如需更新报告，再次运行 `/zz-report`。
```
...
- 用户在 `~/.zz/sessions/` 之外手动放了个文件 → 不读。只读 sessions 目录下的标准文件
...
| 报告中「当前未解决的问题」非空 | 「报告里有 {N} 个未解决问题，下次回来用 `/zz-restore` 接着诊断。」 |
| 报告中所有存档都是 `resolved` | 「这个项目下所有问题都已经诊断完成。如果之后还有新情况，重新走 `/zz-diagnosis`。」 |
