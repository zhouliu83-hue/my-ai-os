...
description: |
  ...
  触发方式：/zz-save、/存档、「保存这次诊断」「记下来」「这个结论留着」
  Save the current diagnosis state to disk for cross-session recall.
  Trigger: /zz-save, "save this diagnosis", "remember this"
---
...
> 现在没什么可存的——前面没做过诊断。先用 `/zz-diagnosis` 或别的诊断 skill 走一轮，再来存档。
...
```
~/.zz/sessions/{slug}/{YYYYMMDD-HHMMSS}-{title-slug}.md
```
...
如果对话里推荐了下一步 skill（比如 zz-diagnosis 末尾推荐了 `/zz-benchmark`），就填上。
如果没有明显推荐，留空。
...
- 用户的诊断信息特别敏感（比如收入数字、商业秘密）→ 提醒一句：「`~/.zz/` 是本地纯文本，没有加密。如果不想存敏感细节，可以手动改一下我刚写的文件。」
...
| 触发条件 | 推荐话术 |
|---|---|
| 用户存完存档，且当前项目下已有 ≥3 份 | 「这个项目下已经攒了 {N} 份存档。如果你想看个总览，输入 `/zz-report` 出一份合并报告。」 |
| 用户存完，但 next_skill 字段非空 | 「下次回来直接走 `/zz-restore`，能直接接到 {next_skill}。」 |
