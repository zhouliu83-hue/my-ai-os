...
description: |
  ...
  触发方式：/zz-deconstruct、/拆概念、「帮我拆解这个概念」「这个词到底什么意思」
  Concept deconstruction using Wittgenstein + Austrian economics framework.
  Trigger: /zz-deconstruct, "deconstruct this concept", "what does this really mean"
---
...
| 触发条件 | 推荐话术 |
|---|---|
| 拆解过程中发现商业模式层面的问题 | 「这个概念背后的问题可能更大，建议 `/zz-diagnosis` 看看商业模式。」 |
...
| 拆解后发现是商业模式问题 | 「概念拆清楚了。想诊断你的具体商业模式？用 `/zz-diagnosis`。」 |
| 拆完概念后发现整句话本身是空转目标，不只是词义模糊 | 「概念清楚了，但你那句话不是目标，是愿望语法。用 `/zz-goal` 审计一下整句。」 |
| 概念拆清楚后，用户还需要把问题变成可推理、可验证的说明书 | 「词清楚了，下一步把问题写清楚。用 `/zz-good-question`。」 |
| 拆解的是用户反复用错的核心词 | 「这个词你之前可能反复踩坑。输入 `/zz-save` 把这次拆解结论存下来，下次再用错时一查就回想起来。」 |
