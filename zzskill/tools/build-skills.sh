...
- **zz-chatroom** — 定向聊天室（推荐专家或指定人物，多角色对话）
- **zz-chatroom-austrian** — 奥派经济聊天室（哈耶克 × 米塞斯 × Claude）
...
- **zz-save** — 存档当前诊断
- **zz-restore** — 恢复上次诊断
- **zz-report** — 多次存档合并成可交付的 markdown 报告

## 进阶-决策系统

- **zz-decision** — 决策系统（把重大决策沉淀成 ~/.zz/decisions/ 下的本地知识工程）
...
- **zz-learning** — 交互式学习（根据上一篇反馈生成下一篇）
...
python3 - "$INNER_DIR" "$OUT_DIR/zzskill-${VERSION}.zip" <<'PY'
import os
import sys
import zipfile
...
echo
echo "done: $OUT_DIR/zzskill-${VERSION}.zip"
