...
description: |
  ...
  触发方式：/zz-content、/内容诊断、「这个内容怎么做」「帮我看看这个文案」
  Content creation diagnosis. After topic passes, diagnose how to turn it into good content.
  Trigger: /zz-content, "how should I create this content", "review my copy"
---
...
| 触发条件 | 推荐话术 |
|---|---|
| 诊断出开头问题 | 「开头有问题。用 `/zz-hook` 优化，生成 10 个方案。」 |
| 诊断出标题问题，或用户明确需要起标题 | 「标题单独做。用 `/zz-xhs-title`。」 |
| 诊断出明显 AI 味，用户想单独拆这个问题 | 「AI 味单独过一遍更清楚。用 `/zz-ai-check`。」 |
| 用户问「为什么没转化」「为什么点进主页不行动」，但没有说清楚断点、目标动作和反馈数据 | 「这个要先把问题说明书写清楚。用 `/zz-good-question`。」 |
| 内容涉及平台选择和运营细节 | 「平台怎么做，去 `/zz-benchmark` 找个对标先模仿。」 |
| 用户在内容方法上走捷径，跳过了该有的判断和积累 | 「你现在不是只差一点内容技巧，是在绕开关键摩擦。试试 `/zz-slowisfast`。」 |
| 内容中使用了模糊概念 | 「这个概念需要先拆清楚，试试 `/zz-deconstruct`。」 |
| 用户做不动、一直在准备 | 「你可能不是内容问题，是执行力问题。试试 `/zz-action`。」 |
...
> 这次诊断的内容卡点和优化方向，输入 `/zz-save` 存档。下次写新内容前 `/zz-restore` 看一眼，避免再踩同样的坑。
