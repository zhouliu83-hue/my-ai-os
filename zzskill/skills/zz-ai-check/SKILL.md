...
description: |
  ...
  触发方式：/zz-ai-check、/AI检测、「帮我看看有没有 AI 味」「检测一下 AI 特征」
  ...
  Trigger: /zz-ai-check, "check for AI writing", "does this sound like AI"
---
...
- 用户的文案本身选题有问题 → 「AI 味不是你最大的问题。选题本身需要重新想。试试 `/zz-content`。」
...
| 触发条件 | 推荐 |
|---|---|
| 检测出开头三件套 | 「开头有套路感。试试 `/zz-hook` 换个开法。」 |
| 文案选题本身有问题 | 「AI 味不是你最大的问题。试试 `/zz-content`。」 |
| 用户想搞清楚自己的文风 | 「先用 `/zz-deconstruct` 拆解你惯用的表达方式，再来改。」 |
| 用户说的概念本身模糊 | 「这个概念先拆清楚再说。试试 `/zz-deconstruct`。」 |
