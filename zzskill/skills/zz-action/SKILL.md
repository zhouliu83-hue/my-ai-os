...
description: |
  ...
  触发方式：/zz-action、/action、「我知道该怎么做但就是不做」「为什么我总是拖延」
  Execution block diagnosis using Adlerian psychology framework.
  Trigger: /zz-action, "I know what to do but can't do it", "why do I procrastinate"
---
...
| 触发条件 | 推荐话术 |
|---|---|
| 用户想行动，但不知道做什么 | 「回到 `/zz-diagnosis` 重新看商业模式，或 `/zz-benchmark` 找个对标。」 |
| 用户的卡点和商业模式本身有关 | 「执行力没问题，问题在商业模式。用 `/zz-diagnosis` 看看。」 |
| 用户做不动是因为目标本身就是空转的（说「我想做有影响力的内容」「想变得更好」这类愿望语法）| 「你做不动不是执行力问题，是目标本身不能驱动行动。先用 `/zz-goal` 把目标审计成可检查的样子。」 |
| 用户不是做不动，而是在关键方法选择上总想找更快的路 | 「你卡的不是行动本身，是方法选错了。试试 `/zz-slowisfast`。」 |
...
> 你今天发现的反向动力和真实目的，输入 `/zz-save` 存档。下次再卡住，`/zz-restore` 把它拉出来对照——很多时候你不是又卡了，是同一个反向动力换了个壳。
