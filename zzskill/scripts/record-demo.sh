#!/usr/bin/env bash
# Record zzskill plugin marketplace demo GIF
...
cleanup_plugins() {
  claude plugin uninstall zz-diagnosis@zhouzheng-skills 2>/dev/null || true
  claude plugin uninstall zz@zhouzheng-skills 2>/dev/null || true
  claude plugin marketplace remove zhouzheng-skills 2>/dev/null || true
}
